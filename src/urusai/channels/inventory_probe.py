"""Inventory probe —— ingest 階段 cheap pre-pass。

決定哪幾本筆記能填、給 channel dispatcher 用。需要系統有 `ffmpeg` / `ffprobe` /
`yt-dlp` CLI 工具。
"""
...
def run_inventory_probe(
    ...
    """Cheap probe → InventoryReport。決定哪幾本筆記能填。"""
    ...
    if has_speech and has_manual_subs:
        ...
        skipped["SubtitleChannel"] = "no speech to subtitle"

    if has_visual and not visual_static:
        dispatched.append("SceneChannel")
    elif not has_visual:
        skipped["SceneChannel"] = "no video stream"
    else:
        skipped["SceneChannel"] = "visual is static / mostly black"
